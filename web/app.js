...
const PATHS = {
  k0Performance:    "/deep_dialog/checkpoints/best/k0/agt_9_performance_records.json",
  k5Performance:    "/deep_dialog/checkpoints/best/k5/agt_9_performance_records.json",
  comparisonDialogue: "/img/k0_vs_k5_dialogue.json",
  k0Cases:          "/img/k0_cases.json",
};
...
function best(s, low = false) {
  ...
  return s.epochs.reduce((b, e, i) => {
    ...
    return low ? (v < b.value ? { epoch: e, value: v } : b)
               : (v > b.value ? { epoch: e, value: v } : b);
...
function createSvg(title, subtitle, seriesList, metric, height = 360) {
  const W = 1160;
  const pad = { left: 78, right: 22, top: 24, bottom: 54 };
  const epochs   = allEpochs(...seriesList.map(s => s.series));
  const allVals  = seriesList.flatMap(s => s.series.values).filter(v => !isNaN(v));
  const minE = epochs[0] ?? 0, maxE = epochs[epochs.length - 1] ?? 1;
  let minV, maxV;
  if (metric === "success_rate") { minV = 0; maxV = 1; }
  else {
    minV = Math.min(...allVals); maxV = Math.max(...allVals);
  ...
  for (let i = 0; i < tc; i++) {
    const e = Math.round(scale(i, 0, tc - 1, minE, maxE));
    const x = xFor(e);
    ticks.push(
  ...
  const legend = seriesList.map((item, idx) => {
    const lx = (item.legendX ?? pad.left) + idx * 0;
    return `<g>
      <rect x="${lx}" y="13" width="14" height="14" rx="4" fill="${item.color}"></rect>
      <text x="${lx + 22}" y="25" fill="${COLORS.text}" font-size="16">${esc(item.label)}</text>
    </g>`;
  }).join("");

  const lines = seriesList.map(item => {
    const path = item.series.epochs.map((e, i) =>
      `${i === 0 ? "M" : "L"} ${xFor(e)} ${yFor(item.series.values[i])}`
    ).join(" ");
...
function informPhrase(k, v) {
  ...
  if (key === "taskcomplete") return val.toLowerCase().includes("match") ? "booking confirmed" : val;
  if (key === "food")         return `${val} food`;
  if (key === "area")         return `in the ${val}`;
  if (key === "price_range")  return `${val} price`;
  if (key === "name")         return val;
  if (key === "address")      return `address: ${val}`;
  if (key === "phone")        return `phone: ${val}`;
  if (key === "postcode")     return `postcode: ${val}`;
  return `${humanKey(k)}: ${val}`;
}

function requestPhrase(k) {
  const key = String(k).toLowerCase();
  if (key === "address")  return "the address";
  if (key === "phone")    return "the phone number";
  if (key === "postcode") return "the postcode";
  if (key === "name")     return "the restaurant name";
...
function toSentence(turn, isUser) {
  ...
  const request = slotList(turn.request_slots, "request");

  if (isUser) {
    if (act === "thanks") return "Thanks!";
    if (act === "deny")   return "That doesn't work for me.";
    if (act === "inform" && inform) return `I'm looking for ${inform}.`;
    if (act === "request" && request) return `Could you tell me ${request}?`;
    if (inform && request) return `I want ${inform} and need ${request}.`;
    if (inform)   return `I'm looking for ${inform}.`;
    if (request)  return `Could you tell me ${request}?`;
    return `${act.replace(/_/g, " ")}.`;
  }

  // agent
  const tc = turn.inform_slots && turn.inform_slots.taskcomplete;
  if (tc && String(tc).toLowerCase().includes("match")) {
    return inform ? `Great news! I found a match — ${inform}.` : "I found a match!";
  }
  if (inform) return `${inform.charAt(0).toUpperCase()}${inform.slice(1)}.`;
  if (request) return `Could you tell me ${request}?`;
  if (act === "no_match") return "Sorry, I couldn't find a match.";
  if (act === "thanks")   return "You're welcome!";
  return `${act.replace(/_/g, " ")}.`;
}
...
function renderTranscript(hist, container, label) {
  container.innerHTML = "";
  if (!hist || !hist.length) {
    container.innerHTML = '<div class="empty-state">No transcript.</div>';
    return;
  }
...
function renderPlots() {
  ...
  root.innerHTML =
    createSvg(`${label} success rate`, "Learning curve",
...
function renderDialogue() {
  const showComparison = state.mode === MODE.COMPARISON && state.comparisonDialogue;
  const showK0Cases    = state.mode === MODE.K0 && state.k0Cases;

  if (!showComparison && !showK0Cases) {
  ...
  if (showComparison) {
    ...
    els.dialogueTitle.textContent = "Same goal — k=0 vs k=5";
    ...
    renderTranscript(c.ddq_history || [], els.k0Transcript, "k=0");
    renderTranscript(c.d3q_history || [], els.k5Transcript, "k=5");
    return;
  }
  ...
  els.dialogueMeta.textContent = [
    `Success: reward=${ok.reward ?? "N/A"}, turns=${ok.turns ?? "N/A"}`,
    `Fail: reward=${fail.reward ?? "N/A"}, turns=${fail.turns ?? "N/A"}`,
  ].join("  •  ");
...
async function init() {
  ...
  try {
    ...
    state.mode = MODE.COMPARISON;
    els.plotsRoot.innerHTML = `<div class="empty-state">Could not load data: ${esc(e.message)}<br><br>Run <code>python generate_demo_dialogue.py</code> first.</div>`;
    renderAll();
  }
}
