"""
A rule-based user simulator for the CamRest676 restaurant domain.

User goals are loaded from camrest_user_goals.p and have the shape
    {'diaact': 'request',
     'inform_slots': {area/food/pricerange: value, ...},
     'request_slots': {name/phone/address/postcode: 'UNK', ...}}
"""
...
class RuleSimulator(UserSimulator):
    ...
    def _sample_action(self):
        ...
        # "sample" a requested slot from the user goal
    ...
    def debug_falk_goal(self):
        """ Debug function: build a fake restaurant goal manually """

        self.goal['inform_slots'].clear()
        self.goal['inform_slots']['area'] = 'centre'
        self.goal['inform_slots']['food'] = 'italian'
        self.goal['inform_slots']['pricerange'] = 'cheap'
        self.goal['request_slots'].clear()
        self.goal['request_slots']['name'] = 'UNK'
        self.goal['request_slots']['phone'] = 'UNK'
    ...
    def response_inform(self, system_action):
        ...
        if 'taskcomplete' in system_action['inform_slots'].keys(): # check all the constraints from agents with user goal
            ...
            if system_action['inform_slots']['taskcomplete'] == dialog_config.NO_VALUE_MATCH:
                self.state['history_slots']['taskcomplete'] = dialog_config.NO_VALUE_MATCH
                self.state['request_slots'].clear()
                    ...
                    if system_action['inform_slots'][slot] == self.goal['inform_slots'][slot]:
                        ...
                        if len(self.state['request_slots']) > 0:
                            ...
                            if len(rest_slot_set) > 0:
                                ...
                                # No remaining slots: just close out politely.
                                self.state['diaact'] = "thanks"
                                self.state['request_slots'].clear()
