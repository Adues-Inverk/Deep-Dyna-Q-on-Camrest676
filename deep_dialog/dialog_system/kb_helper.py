...
class KBHelper:
    ...
    def fill_inform_slots(self, inform_slots_to_be_filled, current_slots):
        ...
        Arguments:
        inform_slots_to_be_filled   --  Something that looks like {area:None, food:None} where area and food are slots that the agent needs filled
        ...
        if dialog_config.auto_suggest == 1:
            print ('Number of restaurants in KB satisfying current constraints: ', len(kb_results))
    ...
    def available_slot_values(self, slot, kb_results):
        ...
        slot_values = {}
        for entry_id in kb_results.keys():
            if slot in kb_results[entry_id].keys():
                slot_val = kb_results[entry_id][slot]
    ...
    def available_results_from_kb(self, current_slots):
        """ Return the available restaurants in the KB based on the current constraints """

        ret_result = []
        current_slots = current_slots['inform_slots']
        constrain_keys = current_slots.keys()

        # Skip control slots and slots the user marked "I do not care".
        constrain_keys = [k for k in constrain_keys if k not in dialog_config.NON_KB_SLOTS]
